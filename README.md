...
## 2. Minh chứng hạ tầng đã triển khai trên GCP

**Máy ảo `ai-gpu-node` (`e2-standard-8`, zone `us-central1-c`) trong Compute Engine:**

![VM instance ai-gpu-node trên Compute Engine](screenshoots/vm_instance.jpg)

**Cloud NAT gateway `ai-nat` — trạng thái `Running` (network `ai-vpc`, router `ai-router`, region `us-central1`):**

![Cloud NAT gateway ai-nat đang chạy](screenshoots/cloud_nat.jpg)

> Hạ tầng gồm VM trong VPC riêng (`ai-vpc`) truy cập Internet ra ngoài qua **Cloud NAT** (không gán IP public trực tiếp cho VM → an toàn hơn), đúng mô hình mạng được yêu cầu trong lab.

## 3. Kết quả benchmark (đo thực tế trên `e2-standard-8`)
| Chỉ số | Giá trị |
|---|---|
...
**Minh chứng chạy benchmark trực tiếp trên terminal của VM:**

![Output benchmark LightGBM trên terminal VM](screenshoots/terminal_benchmark.jpg)
