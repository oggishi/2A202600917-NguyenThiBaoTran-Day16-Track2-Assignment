...
> **⚠️ Lưu ý về hết tài nguyên (stockout):** Họ `n2-standard-8` đôi khi báo lỗi `does not have enough resources available` ở mọi zone trong `us-central1`. Khi đó hãy dùng `e2-standard-8` (cùng 8 vCPU / 32 GB, sẵn có ở mọi zone, rẻ hơn ~$0.27/giờ) — đây là giá trị mặc định hiện tại trong `variables.tf`. Nếu một zone hết chỗ, chỉ cần đổi `TF_VAR_zone` sang zone khác (`us-central1-b/c/f`).
...
> **Tại sao `e2-standard-8`?** Instance này có 8 vCPU và 32 GB RAM, không yêu cầu quota đặc biệt, có sẵn ngay trên tài khoản mới và hiếm khi bị stockout. Chi phí ~$0.27/giờ tại us-central1 — rẻ hơn cả `n2-standard-8` (~$0.382/giờ) lẫn GPU T4 (~$0.54/giờ gồm VM). Nếu muốn số liệu benchmark gần với CPU server cao cấp hơn, có thể dùng `n2-standard-8` hoặc `n2d-standard-8` khi còn quota/tài nguyên.
...
```bash
# Lấy tên instance từ output hoặc gõ trực tiếp (zone phải khớp với output gpu_node_zone)
gcloud compute ssh ai-gpu-node --zone=us-central1-c --tunnel-through-iap --project=<PROJECT_ID_CỦA_BẠN>
```
...
| Precision | 0.9195 |
| Recall | 0.8163 |
| Decision threshold (tối ưu F1) | 0.2518 |
| Inference latency (1 row) | 0.77 ms |
| Inference throughput (1000 rows) | ~283,000 rows/s |

> **Ghi chú:** Số liệu chạy trên `e2-standard-8` (8 vCPU / 32 GB), dataset Kaggle `mlg-ulb/creditcardfraud` (284,807 giao dịch, 492 gian lận = 0.173%). Vì dữ liệu cực mất cân bằng, ngưỡng phân loại được chọn tối ưu F1 trên tập validation (≈0.25) thay vì 0.5 mặc định; AUC-PR là chỉ số phù hợp hơn AUC-ROC cho bài toán này. Kết quả đầy đủ nằm trong `benchmark_result.json`.
...
|---|---|---|
| Compute Engine — CPU Node | `e2-standard-8` | ~$0.27 |
...
4. **Mã nguồn** thư mục `terraform-gcp/` đã chỉnh sửa (comment GPU block, `e2-standard-8`).
5. **Báo cáo ngắn** (5–10 dòng): so sánh kết quả training time, AUC, inference speed; giải thích lý do phải dùng CPU thay GPU.

---

> **Lưu ý cuối (tiếng Việt):** Dù chạy GPU hay CPU, **bước dọn dẹp (Phần 6 — `terraform destroy`) là bắt buộc** ngay sau khi nộp bài. VM `e2-standard-8`, Cloud NAT và External IP vẫn tính phí liên tục theo giây dù không có tác vụ nào đang chạy. Đừng bỏ qua bước này!